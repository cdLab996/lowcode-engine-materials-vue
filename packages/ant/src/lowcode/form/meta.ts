  ...
  configure: {
    ...
    advanced: {
      callbacks: {
        onNodeAdd: (
          dragment: {
            componentMeta: { npm: { package: string | string[] } }
            componentName: string | string[]
            exportSchema: () => any
          },
          currentNode: {
            document: {
              createNode: (arg0: { componentName: string; props: { label: string }; children: any[] }) => any
            }
            replaceChild: (arg0: any, arg1: any, arg2: { reserveSchemaNodeId: boolean }) => void
          }
        ) => {
          const comps = [
            'AInput',
