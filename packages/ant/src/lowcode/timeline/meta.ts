        ...
        setValue(target: { node: any }, value: { key: string | number }[]) {
          ...
          node.children.mergeChildren(
            ...
            (child1: { getPropValue: (arg0: string) => any }, child2: { getPropValue: (arg0: string) => any }) => {
              const a = value.findIndex((item: { key: any }) => String(item.key) === String(child1.getPropValue('key')))
              const b = value.findIndex((item: { key: any }) => String(item.key) === String(child2.getPropValue('key')))
              return a - b
            }
          )
        }
